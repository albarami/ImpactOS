...
class TestScoreMapping:
    ...
    def test_materiality_waiver_required_above_5pct(self, scorer: QualityScorer) -> None:
        ...
        assert "materiality_waiver_required" in result.rules_triggered
        # Warnings are generated by WarningEngine, not the scorer.
    ...
    def test_materiality_critical_above_1pct(self, scorer: QualityScorer) -> None:
        ...
        assert "materiality_critical" in result.rules_triggered
        assert len(result.warnings) == 0
    ...
    def test_materiality_warning_any_unresolved(self, scorer: QualityScorer) -> None:
        ...
        assert "materiality_warning" in result.rules_triggered
        assert len(result.warnings) == 0
    ...
    def test_no_warning_zero_spend(self, scorer: QualityScorer) -> None:
        ...
        assert len(result.warnings) == 0
        assert "materiality_waiver_required" not in result.rules_triggered
        assert "materiality_critical" not in result.rules_triggered
        assert "materiality_warning" not in result.rules_triggered
