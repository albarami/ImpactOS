...
Covers: POST create scenario, POST compile, POST mapping-decisions (bulk),
GET scenario versions, POST lock for governed run.
"""
...
class TestCreateScenarioEndpoint:
    ...
    async def test_create_returns_201(self, client: AsyncClient) -> None:
        response = await client.post("/v1/scenarios", json=_scenario_create_payload())
...
# ===================================================================
# POST /v1/scenarios/{id}/compile
# ===================================================================


class TestCompileEndpoint:
    """POST compile: line items + decisions → ScenarioSpec."""

    @pytest.mark.anyio
    async def test_compile_returns_200(self, client: AsyncClient) -> None:
        # Create scenario first
        create_resp = await client.post("/v1/scenarios", json=_scenario_create_payload())
        ...
        response = await client.post(f"/v1/scenarios/{scenario_id}/compile", json=compile_payload)
...
# ===================================================================
# POST /v1/scenarios/{id}/mapping-decisions
# ===================================================================


class TestMappingDecisionsEndpoint:
    """POST bulk mapping decisions."""
            ...
            "decisions": [
                {
                    ...
                    "decision_type": "APPROVED",
                    "decided_by": str(uuid7()),
                    "rationale": "Correct mapping",
                },
            ],
        }
        response = await client.post(f"/v1/scenarios/{scenario_id}/mapping-decisions", json=payload)
        assert response.status_code == 200
            ...
            "decisions": [
                {
                    ...
                    "decided_by": str(uuid7()),
                    "rationale": "Changed to construction",
                },
            ],
        }
        response = await client.post(f"/v1/scenarios/{scenario_id}/mapping-decisions", json=payload)
...
class TestGetVersionsEndpoint:
    ...
    async def test_versions_returns_200(self, client: AsyncClient) -> None:
        ...
        response = await client.get(f"/v1/scenarios/{scenario_id}/versions")
        assert response.status_code == 200
    ...
    async def test_nonexistent_returns_404(self, client: AsyncClient) -> None:
        response = await client.get(f"/v1/scenarios/{uuid7()}/versions")
        assert response.status_code == 404


# ===================================================================
# POST /v1/scenarios/{id}/lock
# ===================================================================


class TestLockEndpoint:
    """POST lock for governed run."""
    ...
    async def test_lock_returns_200(self, client: AsyncClient) -> None:
        ...
        response = await client.post(f"/v1/scenarios/{scenario_id}/lock", json=payload)
        # Lock with no mappings should succeed (nothing to lock)
        assert response.status_code == 200
    ...
    async def test_lock_increments_version(self, client: AsyncClient) -> None:
        ...
        response = await client.post(
            f"/v1/scenarios/{scenario_id}/lock",
